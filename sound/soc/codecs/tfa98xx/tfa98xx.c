...
#define MAX_COMMAND_BUFFER_LENGTH 150
...
static bool g_nxp_drv_err = true;
...
struct tfa98xx_calibration_data {
	...
	int Fres;			//speaker resonance compensation filter cut-off frequency
	int Qfactory;		//speaker resonance compensation filter Q-factor
};
#define CALIBRATION_DEVICE_NAME "tfa98xx_calibrate\0"
...
static char *fw_name_milan_aac = "milan_nxp/tfa98xx_aac.cnt";
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	pr_info("[NXP DBG] loaded %s - size: %zu\n", fw_name,
					cont ? cont->size : 0);
	...
	pr_info("[NXP DBG] %.2s%.2s\n", container->version, container->subversion);
	pr_info("[NXP DBG] %.8s\n", container->customer);
	pr_info("[NXP DBG] %.8s\n", container->application);
	pr_info("[NXP DBG] %.8s\n", container->type);
	pr_info("[NXP DBG] %d ndev\n", container->ndev);
	pr_info("[NXP DBG] %d nprof\n", container->nprof);
...
{
	char product_id[PRODUCT_IDENTIFIER_BUFF_SIZE] = {0};
	char box_id[PRODUCT_IDENTIFIER_BUFF_SIZE] = {0};
	...
	pr_info("[NXP DBG] %s entry\n", __func__);

	get_product_identifier(product_id);
...
static int tfa98xx_calibration_open(struct inode *inode, struct file *file)
{
...
static ssize_t tfa98xx_calibration_show(struct file *file, char __user *buf,
...
{	
	...
	if (ret) {
		...
		ret = tfaRunWaitCalibration(tfa98xx->handle, &calibrationdone);
		if (ret) {
...
static ssize_t tfa98xx_do_calibration(struct file *file, const char __user *buf,
											size_t count, loff_t *pos)
{
	enum Tfa98xx_Error ret = Tfa98xx_Error_Ok;
	struct tfa98xx *tfa98xx = file->private_data;
	struct i2c_client* i2c = NULL;
	char kernelbuf[MAX_COMMAND_BUFFER_LENGTH] = {0};
	bool isonce = false;
	int ready = 0;
...
static const struct file_operations calibrate_fops={	
	.owner	= THIS_MODULE,
	.open 	= tfa98xx_calibration_open,
	.read 	= tfa98xx_calibration_show,
	.write 	= tfa98xx_do_calibration,
};

int tfa98xx_init_sysfs(struct tfa98xx *tfa98xx)
{
	int ret = 0;
	...
	tfa98xx->calibrate_device.minor = MISC_DYNAMIC_MINOR;
...
void tfa98xx_remove_sysfs(struct tfa98xx *tfa98xx)
{
	...
	misc_deregister(&tfa98xx->calibrate_device);
	return;
}
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (no_start == 0) {
		...
		ret = regmap_read(tfa98xx->regmap, 0x03, &reg);
		if (ret < 0) {
			dev_err(&i2c->dev, "1.Failed to read Revision register: %d\n", ret);
			return -EIO;
		}
		switch (reg & 0xff) {
		case 0x88: /* tfa9888 */
	...
	ret = tfa98xx_init_sysfs(tfa98xx);
	if (ret)
		dev_info(&i2c->dev, "error creating calibration sysfs files\n");
...
bool get_nxp_smartpa_drv_state(void)
{
	return g_nxp_drv_err; 
}
